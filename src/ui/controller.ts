...
const BALL_COLORS = [...COLORS, "gold"] as const; // 코인 반환용(컬러 5 + 궁극)
...
export class Controller {
  ...
  private lanCode = ""; // 현재 소속 방 코드(목록에서 '참가 중' 표시용)
    ...
    try {
      if (this.netMode === "local") {
        ...
        localStorage.setItem(SAVE_KEY + "-host", JSON.stringify({
          snap: serialize(this.state), names: this.playerNames, aiLog: this.aiLog,
          humanSeats: [...this.humanSeats], leftSeats: [...this.leftSeats],
        }));
      }
    } catch { /* 저장 실패 무시 */ }
  }
  ...
  private restoreHostGame(): boolean {
    try {
      ...
      if (!raw) return false;
      const d = JSON.parse(raw) as { snap: Snapshot; names?: string[]; aiLog?: string[]; humanSeats?: number[]; leftSeats?: number[] };
    ...
    return {
      ...
      onJoined: (code, seat, isHost, roster, token) => {
        ...
        if (this.lanResuming) {
          ...
          // 게스트/게임없음: 호스트 상태(resend)를 기다린다 — 인라인 대기 화면(로비)
          this.lanView = "lobby"; this.lanLobbyOpen = false; this.enterLobbyPreview(); this.render(); return;
        }
        // 방 입장 = 인라인 대기 화면(로비)으로. 오버레이(방 목록)는 닫는다.
        this.lanView = "lobby";
        this.lanLobbyOpen = false;
        this.enterLobbyPreview();
        this.render();
      },
   ...
   *  (카드 버림 + 코인은 공급 더미로 반환 + 턴 건너뜀). */
  private onLanRosterChange(): void {
    // 대기실(게임 시작 전): 배경을 방 참가자로 갱신
    if ((this.netMode === "host" || this.netMode === "guest") && this.lanLobbyOpen && this.lanView === "lobby") {
      this.enterLobbyPreview();
    }
    if (this.netMode === "host" && this.state && !this.lanLobbyOpen) {
      ...
      for (const s of [...this.humanSeats]) {
        if (!connected.has(s)) {
          this.eliminatePlayer(s);
          this.humanSeats.delete(s);
          this.pushAiLog(`${this.playerName(s)} 님이 나갔습니다 (카드 폐기·코인 반환).`);
          changed = true;
        }
      }
      // 사람이 혼자만 남으면 게임 종료
      if (changed && !this.state.ended && this.humanSeats.size <= 1) {
        this.state.ended = true;
        this.pushAiLog("상대가 모두 나가 게임이 종료되었습니다.");
      }
      if (changed) {
        this.broadcastState();
        if (this.state.ended) {
          this.phase = "ended";
          this.endOverlayOpen = true;
        } else if (this.leftSeats.has(this.state.currentPlayer)) {
          this.advance(); // 나간 사람 차례였다면 넘긴다
        }
      }
    }
    this.render();
  ...
  /** 호스트가 나가 내가 새 호스트로 승격됨: 권위 상태를 인계받아 게임을 이어간다. */
  private becomeHost(): void {
    ...
    if (!this.state || this.lanLobbyOpen) { this.render(); return; }
    // 로스터에 없는 사람 좌석(이전 호스트 등)을 게임에서 제거
    const connected = new Set(this.lanRoster.map((r) => r.seat));
    ...
    this.pushAiLog("이전 호스트가 나가 당신이 새 호스트가 되었습니다.");
    if (eliminated && !this.state.ended && this.humanSeats.size <= 1) {
      this.state.ended = true;
      this.pushAiLog("상대가 모두 나가 게임이 종료되었습니다.");
    }
    this.lan?.setStatus("playing");
    ...
    // 나간 사람 차례면 넘기고(advance 가 broadcast+startTurn 포함), 아니면 현재 차례부터 구동 재개
    if (this.leftSeats.has(this.state.currentPlayer)) {
      this.advance();
    } else {
      this.broadcastState();
      this.saveGame();
  ...
  /** 대기실 배경 상태: 방 참가자를 빈 게임 판으로 보여준다(실게임 아님, 이전 게임 감추기). */
  private enterLobbyPreview(): void {
    const seed = (Math.random() * 1e9) | 0;
    this.state = createGame(seed, 4, this.mySeat < 0 ? 0 : this.mySeat);
    const byName = new Map(this.lanRoster.map((r) => [r.seat, r.name] as const));
    this.playerNames = [];
    for (let i = 0; i < this.state.numPlayers; i++) this.playerNames[i] = byName.get(i) ?? "대기 중";
  ...
  /** 중도 이탈 플레이어 제거: 코인(구슬)은 공급 더미로 반환, 카드/보너스/보관/퓨전은 폐기. */
  private eliminatePlayer(seat: number): void {
    const p = this.state.players[seat];
    if (!p) return;
    for (const c of BALL_COLORS) { this.state.supply[c] += p.balls[c]; p.balls[c] = 0; }
    p.scored = [];
    p.reserved = [];
    p.fusions = [];
    for (const c of COLORS) p.bonus[c] = 0;
    p.evolutions = 0;
    this.leftSeats.add(seat);
  }
  ...
  /** 내 방(대기실)으로 돌아가기 — 오버레이 닫고 인라인 대기 화면 표시. */
  ...
  private advance(): void {
    ...
    // 게임에서 나간 좌석의 차례는 건너뛴다
    let guard = 0;
    while (!this.state.ended && this.leftSeats.has(this.state.currentPlayer) && guard++ <= this.state.numPlayers) {
      finishTurn(this.state);
    }
  ...
  private renderWaitingLayout(): HTMLElement {
    const inRoom = this.lanJoined && (this.netMode === "host" || this.netMode === "guest");
    const board: (HTMLElement | string)[] = [
      el("div", { class: "waiting-logo" }, [el("i", { class: "fa-solid fa-dragon" })]),
    ];

    if (inRoom && this.netMode === "host") {
      const n = this.lanRoster.length;
      board.push(el("div", { class: "waiting-title" }, ["게임을 시작하세요"]));
      board.push(el("div", { class: "waiting-sub" }, [`${n}명 입장 · 방장이 시작할 수 있어요`]));
      const btns: (HTMLElement | string)[] = [];
      if (n >= 2) btns.push(el("button", { class: "lan-btn primary lg", onclick: () => this.startLanGame(false) },
        [el("i", { class: "fa-solid fa-play mr-1" }), `참가자끼리 시작 (${n}인)`]));
      btns.push(el("button", { class: "lan-btn ghost lg", onclick: () => this.startLanGame(true) },
        [el("i", { class: "fa-solid fa-robot mr-1" }), "AI 채워서 4인 시작"]));
      board.push(el("div", { class: "waiting-actions" }, btns));
      if (n < 2) board.push(el("div", { class: "waiting-hint" }, ["2명 이상 입장하면 사람끼리도 플레이할 수 있어요"]));
      board.push(el("div", { class: "waiting-links" }, [
        el("button", { class: "lan-btn ghost sm", onclick: () => this.openRoomList() }, ["방 목록"]),
        el("button", { class: "lan-btn ghost sm danger", onclick: () => this.leaveLan() }, ["방 나가기"]),
      ]));
    } else if (inRoom) {
      board.push(el("div", { class: "waiting-title" }, ["게임 시작 대기 중"]));
      board.push(el("div", { class: "waiting-sub" }, ["방장이 게임을 시작하면 자동으로 시작됩니다."]));
      board.push(el("div", { class: "waiting-dots" }, [el("span", { class: "loading-dots" }, ["대기 중"])]));
      board.push(el("div", { class: "waiting-links" }, [
        el("button", { class: "lan-btn ghost sm", onclick: () => this.openRoomList() }, ["방 목록"]),
        el("button", { class: "lan-btn ghost sm danger", onclick: () => this.leaveLan() }, ["방 나가기"]),
      ]));
    } else {
      board.push(el("div", { class: "waiting-title" }, ["방에 참가해 주세요"]));
      board.push(el("div", { class: "waiting-sub" }, ["방을 만들거나 목록에서 참가하세요."]));
      board.push(el("div", { class: "waiting-actions" }, [
        el("button", { class: "lan-btn primary lg", onclick: () => this.openRoomList() },
          [el("i", { class: "fa-solid fa-network-wired mr-1" }), "방 목록 열기"]),
      ]));
    }
    ...
    return el("div", { class: "game-layout" }, [
      el("div", { class: "game-left" }, [
        ...
        el("div", { class: "waiting-board" }, board),
      ]),
      this.renderWaitingRight(),
    ]);
  }

  /** 방 목록 오버레이 열기(대기 화면 위에). */
  ...
  private renderRoomListBody(): (HTMLElement | string)[] {
    ...
    if (inRoom) {
      // 이미 방에 있음 → 아래 '내 방으로'로 대기실 복귀. 다른 방 생성/참가는 나간 뒤 가능.
      body.push(el("div", { class: "lan-hint" }, ["이미 방에 참가 중입니다. 다른 방에 가려면 먼저 '방 나가기'로 떠나세요."]));
    } else {
      // 닉네임 + 새 방 만들기
      body.push(el("input", {
    ...
    if (attached) {
      actions.push(el("button", { class: "lan-btn ghost", onclick: () => this.backToMyRoom() }, [
        el("i", { class: "fa-solid fa-right-to-bracket mr-1" }), "내 방으로",
      ]));
  ...
  private renderRoomRow(r: RoomInfo, inRoom: boolean): HTMLElement {
    ...
    const mine = r.code === this.lanCode && (this.lanJoined || this.netMode === "spectator");
    const canJoin = !inRoom && !playing && !full;
    return el("div", { class: `lan-room${mine ? " mine" : ""}` }, [
      el("div", { class: "lan-room-info" }, [
        el("div", { class: "lan-room-name" }, [
          ...
          mine ? el("span", { class: "lan-room-badge mine" }, ["참가 중"]) : "",
        ]),
  ...
  private renderRoomLobbyBody(): (HTMLElement | string)[] {
    ...
    for (let s = 0; s < 4; s++) {
      ...
      if (entry) {
        ...
        if (s === (this.lan?.hostSeat ?? 0)) chips.push(el("span", { class: "lan-chip host" }, ["호스트"]));
  ...
  private renderHeader(): HTMLElement {
    ...
    // 새 게임: 로컬/호스트만(게스트·관전자는 불가)
    if (this.netMode === "local" || this.netMode === "host") children.push(newGameBtn);
  ...
  private renderAiPanel(index: number): HTMLElement {
    ...
    // Name + points. LAN 사람 상대는 사람(🧑), 나간 좌석은 '나감' 으로 구분.
    const left = this.leftSeats.has(index);
    const isHumanOpp = this.humanSeats.has(index);
    if (left) panel.classList.add("left-seat");
    const roleBadge = left
      ? el("span", { class: "badge badge-xs badge-ghost" }, ["🚪 나감"])
      : isHumanOpp
    ...
    panel.append(el("div", { class: "flex items-center justify-between" }, [
      el("div", { class: "flex items-center gap-2" }, [
        el("div", { class: "avatar placeholder" }, [
          el("div", {
            class: `${isHumanOpp && !left ? "bg-info text-info-content" : "bg-neutral text-neutral-content"} w-6 rounded-full`,
          }, [
            el("i", { class: `fa-solid ${left ? "fa-xmark" : isHumanOpp ? "fa-user" : "fa-robot"} text-xs` }),
          ]),
        ]),
